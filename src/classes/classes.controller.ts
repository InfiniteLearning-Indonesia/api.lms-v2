...
export class ClassesController {
  ...
  async assignBatchMentors(
    @Req() req: any,
    @Body() body: { programId: string; mentorIds: string[] },
  ) {
    return this.classesService.assignBatchMentors(
      req.params.batchId,
      body.programId,
      body.mentorIds,
    );
  ...
  @Roles('admin')
  async saveMentorMatrix(
    @Param('batchId') batchId: string,
    @Body() body: { matrix: Record<string, any> },
  ) {
  ...
  @Roles('admin')
  async createProgramBatch(
    @Body() body: { programId: string; batchName: string },
  ) {
  ...
  async assignMentor(@Body() body: { mentorId: string; programName: string }) {
    return this.classesService.assignMentorToProgram(
      body.mentorId,
      body.programName,
    );
  }
  ...
  async assignMentorByProgramId(
    ...
    return this.classesService.assignMentorToProgramById(
      programId,
      body.mentorId,
    );
  }
  ...
  async updateCompetency(
    @Req() req: any,
    @Body()
    body: { name?: string; category?: string; programCompetencyId?: string },
  ) {
    return this.classesService.updateCompetency(
      req.user.id,
      req.params.id,
      body,
    );
  ...
  async getAllRubrikAssessments(@Req() req: any) {
    return this.classesService.getRubrikAssessmentsByProgram(
      req.query?.programId,
    );
  }

  @Get('programs/:programId/rubrik-assessments')
  async getRubrikAssessmentsByProgram(
    @Req() req: any,
    @Param('programId') programId: string,
  ) {
  ...
  async remapClassCompetencies(
    ...
    return this.classesService.remapClassCompetencies(
      req.user.id,
      classId,
      body.remappingData,
    );
  ...
  async upsertCompetencyScore(
    ...
    return this.classesService.upsertCompetencyScore(
      body.studentId,
      body.competencyId,
      body.score,
    );
  ...
  async importAssessmentScores(
    ...
    return this.classesService.importAssessmentScores(
      req.user.id,
      programId,
      body.scores,
    );
  ...
  async updateMaterial(
    ...
    return this.classesService.updateMaterial(req.user.id, id, body);
  }

  @UseGuards(SessionAuthGuard)
  @Get(':classId/material/:materialId')
  async getMaterialDetails(@Req() req: any) {
    return this.classesService.getMaterialDetails(
      req.params.classId,
  ...
  async updateAssignment(
    ...
    return this.classesService.updateAssignment(req.user.id, id, body);
  }

  @UseGuards(SessionAuthGuard)
  @Get(':classId/assignment/:assignmentId')
  async getAssignmentDetails(@Req() req: any) {
    return this.classesService.getAssignmentDetails(
      req.params.classId,
  ...
  async handoverMentor(
    ...
    return this.classesService.handoverMentor(
      body.oldMentorId,
      body.newMentorId,
      body.programId,
    );
  ...
  @Post('user-batches')
  @Roles('admin')
  async updateUserBatches(
    @Body() body: { userId: string; batchIds: string[] },
  ) {
  ...
  async updateCompetencyRubric(
    ...
    return this.classesService.updateCompetencyRubric(
      competencyId,
      body.rubric,
    );
  }
  ...
  async gradeSubmission(
    ...
    return this.classesService.gradeSubmission(
      submissionId,
      req.user.id,
      body.score,
      body.manualFeedback,
  ...
  @Roles('mentor', 'admin')
  @Post(
    ':classId/assignment/:assignmentId/submissions/:submissionId/ai-evaluate',
  )
  ...
  async fetchAiModels(
    @Body() body: { provider: string; hostOrApiKey?: string },
  ) {
  ...
  async bulkAiEvaluateSubmissions(
    ...
    return this.classesService.bulkAiEvaluateSubmissions(
      req.user.id,
      assignmentId,
      body,
    );
  ...
  async updateAssignmentWeights(
    @Body() body: { updates: Array<{ id: string; weight: number }> },
  ) {
  ...
  @Roles('mentor')
  async getMentorStudentLogbooks(
    @Req() req: any,
    @Param('batchId') batchId: string,
  ) {
  ...
  async reviewLogbook(
    ...
    @Body() body: { status: any; feedback?: string },
  ) {
    return this.classesService.reviewLogbook(
      req.user.id,
      logbookId,
      body.status,
      body.feedback,
  ...
  @Roles('mentor')
  async toggleMentorAsyncDay(
    @Req() req: any,
    @Body() body: { date: string; note?: string },
  ) {
    return this.classesService.toggleMentorAsyncDay(
      req.user.id,
      body.date,
      body.note,
    );
